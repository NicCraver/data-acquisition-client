    ...
    (config) => {
      if (requestInterceptorSuccessCb) {
        ...
        if (!DataType.isObject(_config))
          throw new Error('requestInterceptorSuccessCb必须返回一个config对象')
  ...
  instance.interceptors.response.use(
    (res) => {
      if (responseInterceptorSuccessCb) {
        console.log(
          `%c${'[HTTP]'} %c${dayjs().format('HH:mm:ss')} %c${res.config.method.toLocaleUpperCase()} %c${
        ...
        if (!DataType.isObject(_res))
          throw new Error('responseInterceptorSuccessCb必须返回一个response对象')

        return _res
      }
      console.log(
        `%c${'[HTTP]'} %c${dayjs().format('HH:mm:ss')} %c${res.config.method.toLocaleUpperCase()} %c${res.config.url}(%c${res.status}` + '%c)',
        'font-weight:bold',
        'font-weight:bold',
        'font-weight:bold',
        'font-weight:bold',
    ...
    (err) => {
      ...
      const err_code = errres?.status ?? 0
      let err_msg = ''
      // 收集错误信息
      switch (err_code) {
        case 400:
        ...
        case 404:
          err_msg = `请求地址出错: ${errres?.config?.url ?? '/'}`
          break
      ...
      if (config.__retryCount >= config.retry) {
        ...
        if (responseInterceptorErrorCb) {
          if (err.code === 'ECONNABORTED' || err.message === 'Network Error' || err.message.includes('timeout')) {
            console.log(
              `%c${'[HTTP]'} %c${dayjs().format('HH:mm:ss')} %c${err.config.method.toLocaleUpperCase()} %c${
          ...
          const _res = responseInterceptorErrorCb({ ...err }, err_code, err_msg)
          if (!DataType.isObject(err?.config))
            throw new Error('responseInterceptorErrorCb必须返回一个err包含{config,response}')
